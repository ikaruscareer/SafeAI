"""OpenClaw configuration-file framework adapter.

OpenClaw projects may declare agent authority in ``.openclaw/config.json``,
other JSON/YAML files below ``.openclaw/``, or a root ``.openclaw.yaml``.
The adapter parses structured configuration first and falls back to scanning
the raw text when a file is empty or malformed.
"""
...
_INSTRUCTION_KEYS = ("systemPrompt", "system_prompt", "instructions", "prompt", "content")
_TOOL_KEYS = ("tools", "allowed_tools", "allowedTools", "permissions", "allow")
_MCP_KEYS = ("mcpServers", "mcp_servers")
...
_OPENCLAW_FILE_RE = re.compile(
    r"(^|/)\.openclaw/.+\.(json|ya?ml)$", re.IGNORECASE,
)


def _scan_capabilities(text, caps, evidence_prefix):
    """Scan config text for capability keywords, appending to *caps*."""
    patterns = (
...
def _load_structured(content):
    """Try JSON then YAML and return a mapping, or ``None``."""
    try:
...
def _source_name(path):
    ...
    if marker in normalized:
        return ".openclaw/" + normalized.split(marker, 1)[1]
...
class OpenClawParser:
    """Parse OpenClaw JSON and YAML agent configuration."""
    ...
    def detect(self, path, content, scan_ctx=None):
        ...
        return basename in {".openclaw.yaml", ".openclaw.yml"} or bool(
            _OPENCLAW_FILE_RE.search(normalized)
        )
